...
ElasticSearch.prototype.buildQueryPlan = function(query, callback) {
  ...
  query.metrics.forEach(function(metric) {
    ...
    if (!metric.formula && metric.collection) {
      ...
      colQuery.key = self.common.hash(colQuery.type + '_' + metric.collection.key + '_' + JSON.stringify(_$match) + '_' + JSON.stringify(_$unwind) + '_' + metric.key + '_' + metric.name);
...
<<<<<<< 94b9e71e38f1a08d65570beca5e3cce58e028e2a
            field: Array.isArray(metric.dependsOn) ? metric.dependsOn[0] : (metric.dependsOn || metric._key)
=======
            field: Array.isArray(metric.dependsOn) ? metric.dependsOn[0] : (metric.dependsOn || metric.attribute || metric._key),
            key: metric.key
>>>>>>> c903facaf9f83ce0738a86917bc766ed3edc73d4
          };
        }
      }
...
<<<<<<< 94b9e71e38f1a08d65570beca5e3cce58e028e2a
                  if (typeof(metric[aggregation]) === 'object') {
                    //result[metric[aggregation].name || 'agg'] = {};
                    result = field;
                    //delete metric[aggregation].name;
                  } else {
