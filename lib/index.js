    ...
    if (!metric.formula && metric.collection) {
      ...
      if (metric.key !== 'fake') {
        ...
        if (metric.aggregation === 'count')
          ...
          _$group.aggs[metric.key].cardinality = {field: metric.dependsOn[0], precision_threshold: 100};
        }
        else {
          _$group.aggs[metric.key] = {};
  ...
  return self.buildQueryPlan(query, function (err, plan) {
    async.mapSeries(Object.keys(plan.colQueries), function (key, cb) {
      ...
      var searchObject = {
        ...
        query: {
          filtered: {
            query: queryPlan.$match.filter//{match_all: {}},
                  ...
                  result[field].terms.format = function () {
                    switch (query.interval) {
                      ...
                      case 'timebucket.second':
                        return 'yyy-mm-dd\'T\'HH:mm:ss.000Z';
                      case 'timebucket.minute':
                        return 'yyy-mm-dd\'T\'HH:mm:00.000Z';
                      case 'timebucket.hour':
                        return 'yyy-mm-dd\'T\'HH:00:00.000Z';
                      case 'timebucket.date':
                      case 'timebucket.day':
                      case 'timebucket.ddata':
                        return 'yyy-mm-dd\'T\'00:00:00.000Z';
                      case 'timebucket.month':
                        return 'yyy-mm-01\'T\'00:00:00.000Z';
                      case 'timebucket.year':
                        return 'yyy-01-01\'T\'00:00:00.000Z';
                      default:
                        return 'MM-yyyZ';
                    }
                  }()
      ...
      results.forEach(function (result) {
        ...
        if (query.dimensions.length > 0) {
          ...
          var row = ce.clone(templateRow);
          query.metrics.forEach(function (m) {
            var value = result.aggregations[m.key];
            if (value) {
